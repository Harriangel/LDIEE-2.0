...
foreach ($equipment_id as $equip_id){
  ...
  while ($history = $stm->fetch(PDO::FETCH_ASSOC)){
      $info_equipment[] = '<li>'.$history['comments'].'</li>';
  }
...
// Cuerpo del mensaje
$body = '<h2>Se ha desactivado su prestamo</h2>
...
$mail->Subject = 'Desactivacion de prestamo';
$mail->isHTML(true);
$mail->Body = $body;
$mail->AddAddress($email);
$_SESSION['success'] = $mail->send() ? 'Prestamo Desactivado' : 'Correo no enviado';
$mail->smtpClose();
?>